...
export default function SMSCommunicationsContent() {
  ...
  const handleSaveConfig = () => {
    if (!selectedTenant || !phoneNumber || !provider) {
      setTestStatus("error")
  ...
  const handleTestSMS = async () => {
    if (!testPhone) {
      setTestStatus("error")
      setTestMessage("Please enter a test phone number")
      return
    }

    setTestStatus("loading")
    setTestMessage("Sending test SMS...")

    setTimeout(() => {
      setTestStatus("success")
      setTestMessage(`Test SMS sent successfully to ${testPhone}!`)
      setTimeout(() => setTestStatus("idle"), 3000)
    }, 2000)
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-3xl font-bold text-foreground">SMS Communications</h1>
          <p className="text-muted-foreground">Configure SMS services for your tenants</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <Card className="lg:col-span-1 bg-card border-border">
          <CardHeader>
            ...
            <CardDescription>Set up SMS provider for a tenant</CardDescription>
          </CardHeader>
        ...
        <Card className="lg:col-span-1 bg-card border-border">
          <CardHeader>
            <CardTitle className="text-foreground">Test SMS</CardTitle>
            <CardDescription>Send a test SMS to verify configuration</CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            <div className="space-y-2">
              <Label htmlFor="test-phone" className="text-foreground">
                Test Phone Number
              </Label>
              <Input
                id="test-phone"
            ...
            >
              ...
              {testStatus === "loading" ? "Sending..." : "Send Test SMS"}
            </Button>
          </CardContent>
        </Card>
            ...
            <div className="space-y-3">
              {configs.map((config) => (
                <div key={config.tenantId} className="p-3 bg-background rounded-lg border border-border">
                  ...
                  <p className="text-xs text-muted-foreground mt-1">{config.phoneNumber}</p>
                  <div className="flex items-center justify-between mt-2">
                    <span className="text-xs bg-primary/10 text-primary px-2 py-1 rounded">
                      {smsProviders.find((p) => p.value === config.provider)?.label}
                    </span>
