/**
 * Apply Notification Tables Migration to All Tenant Schemas
 * Team: Epsilon
 * Purpose: Create notification tables in all existing tenant schemas
 */
...
async function applyNotificationMigration() {
  ...
  try {
    ...
    const migrationPath = path.join(__dirname, '../migrations/1731760000000_create-notification-tables.sql');
    ...
    // Create global notification type defaults table
    console.log('📦 Creating global notification type defaults table...');
    ...
    await client.query(`
      CREATE TABLE IF NOT EXISTS notification_type_defaults (
        ...
        default_sms_enabled BOOLEAN DEFAULT false,
    ...
    await client.query(`
      INSERT INTO notification_type_defaults (notification_type, display_name, description, default_email_enabled, default_sms_enabled, default_push_enabled, default_in_app_enabled)
      VALUES
        ('critical_alert', 'Critical Alerts', 'Emergency and critical patient alerts', true, true, true, true),
        ('appointment_reminder', 'Appointment Reminders', 'Upcoming appointment notifications', true, true, true, true),
        ('lab_result', 'Lab Results', 'Laboratory test results available', true, false, true, true),
        ('billing_update', 'Billing Updates', 'Billing and payment notifications', true, false, false, true),
        ('staff_schedule', 'Staff Schedule', 'Schedule changes and shift updates', true, false, true, true),
        ('inventory_alert', 'Inventory Alerts', 'Low stock and inventory warnings', false, false, false, true),
        ('system_maintenance', 'System Maintenance', 'System maintenance and downtime notices', true, false, true, true),
        ('general_info', 'General Information', 'General hospital announcements', false, false, false, true)
      ON CONFLICT (notification_type) DO NOTHING
    `);
    
    console.log('   ✅ Notification type defaults created');
    console.log('');
