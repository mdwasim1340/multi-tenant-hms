# Day 2, Task 3: Patient Service Layer

## 🎯 Task Objective
Create the PatientService class with database operations and business logic.

## ⏱️ Estimated Time: 2 hours

## 📝 Step 1: Create Service File

Create file: `backend/src/services/patient.service.ts`

```typescript
import { Pool, PoolClient } from 'pg';
import { Patient, CreatePatientData, UpdatePatientData, PatientSearchQuery } from '../types/patient';
...
export class PatientService {
  ...
  async createPatient(
    ...
    try {
      ...
      return await this.getPatientById(patient.id, tenantId, client);
  ...
  async updatePatient(
    ...
    try {
      ...
      return await this.getPatientById(patientId, tenantId, client);
  ...
  private async saveCustomFields(
    ...
    if (values.length > 0) {
      ...
      const params = values.flatMap(v => [v.entity_type, v.entity_id, v.field_id, v.value]);
...
Create file: `backend/src/services/__tests__/patient.service.test.ts`
...
describe('PatientService', () => {
  ...
  describe('createPatient', () => {
    it('should create a patient successfully', async () => {
      ...
      expect(patient.patient_number).toBe(patientData.patient_number);
      expect(patient.first_name).toBe(patientData.first_name);
    });
    
    it('should throw error for duplicate patient number', async () => {
      const patientData = {
        patient_number: 'DUPLICATE001',
        first_name: 'Test',
        last_name: 'Patient',
        date_of_birth: '1990-01-01T00:00:00.000Z'
      };
      
      await service.createPatient(patientData, 'demo_hospital_001', 1);
      
      await expect(
        service.createPatient(patientData, 'demo_hospital_001', 1)
      ).rejects.toThrow('DUPLICATE_PATIENT_NUMBER');
    });
  });
});
```
